...
(function(){
  ...
  function render(){
    ...
    el.meta.textContent = Seed:  • Pool size:  • Progress stored locally;
  ...
  function showFeedback(pct, correct, total, elapsedSec){
    el.result.innerHTML = 
      <div class="row">
        <div class="kpi"><div class="label">Score</div><div class="value">%</div></div>
        <div class="kpi"><div class="label">Correct</div><div class="value"> / </div></div>
        <div class="kpi"><div class="label">Time</div><div class="value"></div></div>
      </div>
      <div class="hr"></div>
      <div class="small">Review each question below: your choice, the correct answer, and why.</div>
    ;
  ...
  function evaluate(){
    ...
    state.questions.forEach((q)=>{
      ...
      perQuestion.push({ id: q.id, chosenIndex: chosen ?? null, correctIndex: q.answerIndex, isCorrect });
    });
    ...
    mod.history.push({ at: mod.lastAttemptAt, seed: state.seed, scorePct: pct, correct, total, elapsedSec, perQuestion });

    if(mod.bestScorePct == null || pct > mod.bestScorePct) mod.bestScorePct = pct;
    mod.completed = pct >= 70;
  ...
  async function init(){
    ...
    el.btnSubmit.addEventListener('click', ()=> { if(!state.evaluated) evaluate(); });
